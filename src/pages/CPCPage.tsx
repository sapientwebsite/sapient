...
const ManagerCard = ({
  ...
  <div className="flex flex-col gap-3 sm:gap-4 w-[min(85vw,340px)] sm:w-[360px] lg:w-[400px] shrink-0">
...
export default function CPCPage() {
  ...
  const scroll = (direction: 'left' | 'right') => {
    ...
    if (!el) return;
    const firstCard = el.querySelector<HTMLElement>('[data-cpc-card]');
    const gap = 24;
    const amount = (firstCard?.offsetWidth ?? 340) + gap;
    el.scrollBy({
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-[#FDFCF9]" style={{ backgroundImage: 'url(/sunburst-hq.png)', backgroundRepeat: 'no-repeat', backgroundPosition: 'center top', backgroundSize: 'cover' }}>
      ...
      <main className="w-full max-w-[1440px] mx-auto pt-10 sm:pt-16 lg:pt-20 pb-24 sm:pb-32 lg:pb-40 px-4 sm:px-6 md:px-12 lg:px-20 relative">
        <div className="flex flex-col gap-10 sm:gap-12 lg:gap-16">
          ...
          >
            {managers.map((m, i) => (
              <div key={`${m.name}-${m.date}-${i}`} data-cpc-card>
                <ManagerCard {...m} />
              </div>
            ))}
          </div>
